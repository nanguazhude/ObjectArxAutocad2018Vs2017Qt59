varAns.emplace(LR"(虚线层)"sv, ApplyMaps::value_type::second_type{ [](simple_code_args) {
	...
	/*if (varLocalLTR)*/ {
		{
			...
			if (Acad::eOk == argLTT->getAt(LR"(_虚线_)", varLTypeIDTmp)) {
				varLTypeID = varLTypeIDTmp;
			}
			else {
				acutPrintf(LR"(找不到线型：_虚线_
)");
			}
		}
	}
	...
	argLTR->setDescription(LR"(图层：虚线层(兼容旧版本))")/*注释*/;
} ,false });
